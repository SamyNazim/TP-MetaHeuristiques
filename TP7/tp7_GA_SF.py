...
def GA_FS(X, y, alpha, N, T, Rc, Rm,
    ...
    Follows exactly the pseudo-code from TP N°7:
      1. Generate a random binary population P of N solutions.
      2. t ← 0
      3. Repeat:
           P_new ← ∅
           For i = 1 to N DIV 2:
             - Selection: randomly pick two parents P1, P2
             - Crossover (prob Rc): 1-point crossover at random k
             - Mutation (prob Rm per bit): bit-flip
             - [Repair if Fixed SF]
             - Add C1, C2 to P_new
           Replacement: P ← P_new
           Evaluate P; update best solution
           t ← t + 1
         Until t = T  OR  best unchanged for 3 consecutive iterations
    """
    D_feat = X.shape[1]
    ...
    for t in range(T):
        ...
        for _ in range(N // 2):
            ...
            # Repair: enforce exactly SF ones when in Fixed mode
            if use_fixed:

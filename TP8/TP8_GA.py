...
def _repair_sf(child, SF):
    """
    Repair a binary chromosome so it has exactly SF ones.
    - If too many 1s: randomly zero out the excess.
    - If too few  1s: randomly set deficit zeros to 1.
    """
    child = child.copy()
    ones  = np.where(child == 1)[0]
    zeros = np.where(child == 0)[0]
    n_ones = len(ones)
    if n_ones > SF and len(ones) > 0:
        to_remove = np.random.choice(ones, n_ones - SF, replace=False)
        child[to_remove] = 0.0
    elif n_ones < SF and len(zeros) >= SF - n_ones:
        to_add = np.random.choice(zeros, SF - n_ones, replace=False)
        child[to_add] = 1.0
    return child


def _roulette_select(pop_fitness):
    """
    Roulette-wheel selection for minimization.
    Converts fitness to selection probability using inverted scores:
      score_i = max(f) - f_i + epsilon   (higher score = lower fitness = better)
    Returns the index of the selected individual.

    Pseudo-code (TP N°8 – Variant 1):
      s ← 0
      For j ← 1 to N do
        s ← s + f(x^(j))
      q ← []
      For j ← 1 to N do
        if j == 1: q[j] ← f(x^(j)) / s
        else:      q[j] ← q[j-1] + f(x^(j)) / s
      r ~ U[0,1]
      Select x^(p) where q[p-1] < r < q[p]
    """
    N = len(pop_fitness)
    scores = np.max(pop_fitness) - pop_fitness + 1e-10  # invert: lower fitness → higher score
    total  = np.sum(scores)
    probs  = scores / total

    # Build cumulative distribution q
    q = np.cumsum(probs)
...
def _crossover_1point(parent1, parent2, D_feat):
    """
    1-Point crossover:
      Choose a random crossover point k;
      C1 ← P1[0:k] + P2[k:]
      C2 ← P2[0:k] + P1[k:]
    """
    k = np.random.randint(1, D_feat)
    child1 = np.concatenate([parent1[:k], parent2[k:]])
    child2 = np.concatenate([parent2[:k], parent1[k:]])
    return child1, child2


def _crossover_2point(parent1, parent2, D_feat):
    """
    2-Point crossover:
      Choose random crossover points k1 and k2 (k1 < k2);
      C1 ← P1[0:k1] + P2[k1:k2] + P1[k2:]
      C2 ← P2[0:k1] + P1[k1:k2] + P2[k2:]
    """
    pts  = sorted(np.random.choice(range(1, D_feat), 2, replace=False))
    k1, k2 = pts[0], pts[1]
...
def GA_FS(X, y, alpha, N, T, Rc, Rm,
    ...
    Supports all three variants from TP N°8:

    Variant 1 — Roulette Wheel selection + 1-Point crossover + Children replacement
    Variant 2 — Roulette Wheel selection + 1-Point OR 2-Point crossover + Children replacement
    Variant 3 — Roulette Wheel selection + 1-Point OR 2-Point crossover + Elitist replacement
                 (N Best from union of P and P_new)

    sf_mode = "Random" : standard binary encoding, SF evolves freely.
    sf_mode = "Fixed"  : every chromosome is initialised with exactly SF ones
                         and repaired to SF ones after each crossover/mutation.

    Stagnation stop: best unchanged for 3 consecutive iterations.
    """
    D_feat   = X.shape[1]
    use_fixed = (sf_mode == "Fixed") and (SF is not None) and (0 < SF <= D_feat)

    # ── 1. Initialise binary population ───────────────────────────────
    if use_fixed:
        pop = np.zeros((N, D_feat))
        for i in range(N):
            chosen = np.random.choice(D_feat, SF, replace=False)
            pop[i, chosen] = 1.0
    else:
    ...
    for t in range(T):
        ...
        for _ in range(N // 2):
            ...
            if selection_method == "Roulette Wheel":
                # Variant 1/2/3: fitness-proportionate (roulette wheel)
                p1_idx = _roulette_select(pop_fitness)
                p2_idx = _roulette_select(pop_fitness)
                # avoid identical parents when possible
                attempts = 0
                while N > 1 and p2_idx == p1_idx and attempts < 10:
                    p2_idx = _roulette_select(pop_fitness)
                    attempts += 1
            else:
                # Original variant: uniform random selection
            ...
            if r < Rc:
                if crossover_method == "1-Point":
                    # Variant 1/2 (1-point branch):
                    # C1 ← P1[0:k] + P2[k:]
                    # C2 ← P2[0:k] + P1[k:]
                    child1, child2 = _crossover_1point(parent1, parent2, D_feat)

                elif crossover_method == "2-Point":
                    # 2-Point only:
                    # C1 ← P1[0:k1] + P2[k1:k2] + P1[k2:]
                    # C2 ← P2[0:k1] + P1[k1:k2] + P2[k2:]
                    child1, child2 = _crossover_2point(parent1, parent2, D_feat)

                else:
                    # Variant 2 ("1-Point + 2-Point"):
                    # Randomly choose between 1-point and 2-point each time
            ...
            # ── Repair: enforce exactly SF ones in Fixed mode ──────────
            if use_fixed:
                child1 = _repair_sf(child1, SF)
                child2 = _repair_sf(child2, SF)
        ...
        P_new_arr     = np.array(P_new[:N])
        P_new_fitness = np.array(
            [fitness_ga_fs(P_new_arr[i], X, y, alpha, k_knn)[0] for i in range(N)]
        )

        # ── Replacement ───────────────────────────────────────────────
        if replacement_method == "Elitist":
            # Variant 3: P ← N Best solutions from union(P, P_new)
            union_pop     = np.vstack([pop, P_new_arr])
            union_fitness = np.concatenate([pop_fitness, P_new_fitness])
            best_indices  = np.argsort(union_fitness)[:N]
            pop           = union_pop[best_indices]
            pop_fitness   = union_fitness[best_indices]
        else:
            # Variant 1 & 2: P ← P_new  (full children replacement)
            pop         = P_new_arr
            pop_fitness = P_new_fitness
...
with st.container(border=True):
    ...
    with ga_left_col:
        ...
        ga_sfmode_col, ga_sfval_col = st.columns(2)
        with ga_sfmode_col:
            st.markdown("SF Mode")
            ga_sf_mode = st.selectbox(
                "SF Mode", ["Random", "Fixed"],
                key="ga_sf_mode", label_visibility="collapsed"
            )
        with ga_sfval_col:
            st.markdown("SF (if Fixed)")
            ga_SF = st.number_input(
                "SF GA", min_value=1, max_value=200, value=10,
                key="ga_SF", label_visibility="collapsed",
                disabled=(ga_sf_mode == "Random")
            )

        st.markdown("**GA parameters**")

        # ── GA Variant selector ────────────────────────────────────────
        st.markdown("GA Variant")
        ga_variant = st.selectbox(
            "GA Variant",
            [
                "Variant 1",
                "Variant 2",
        ...
        # Derive operator defaults from variant
        if ga_variant == "Variant 1 — Roulette Wheel + 1-Point + Children":
            _default_sel   = "Roulette Wheel"
            _default_cross = "1-Point"
            _default_rep   = "Children"
        elif ga_variant == "Variant 2 — Roulette Wheel + 1/2-Point + Children":
            _default_sel   = "Roulette Wheel"
            _default_cross = "1-Point + 2-Point"
            _default_rep   = "Children"
        elif ga_variant == "Variant 3 — Roulette Wheel + 1/2-Point + Elitist":
            _default_sel   = "Roulette Wheel"
            _default_cross = "1-Point + 2-Point"
            _default_rep   = "Elitist"
        else:
            _default_sel   = "Random"
            _default_cross = "1-Point"
            _default_rep   = "Children"
        ...
        with ga_sel_col:
            ...
            ga_selection = st.selectbox(
                ...
                index=["Random", "Roulette Wheel"].index(_default_sel),
                key="ga_selection",
                label_visibility="collapsed",
                disabled=(ga_variant != "Custom"),
            )
        with ga_cross_col:
            ...
            ga_crossover = st.selectbox(
                ...
                index=["1-Point", "2-Point", "1-Point + 2-Point"].index(_default_cross),
                key="ga_crossover",
                label_visibility="collapsed",
                disabled=(ga_variant != "Custom"),
            )
        with ga_rep_col:
            ...
            ga_replacement = st.selectbox(
                "Replacement",
                ["Children", "Elitist"],
                index=["Children", "Elitist"].index(_default_rep),
                key="ga_replacement",
                label_visibility="collapsed",
                disabled=(ga_variant != "Custom"),
            )
...
# -------------------------------------------------------
# Resolve effective operators from variant choice
# -------------------------------------------------------

def _resolve_ga_ops(variant, sel, cross, rep):
    """Return the actual (selection, crossover, replacement) tuple to use."""
    if variant == "Variant 1 — Roulette Wheel + 1-Point + Children":
        return "Roulette Wheel", "1-Point", "Children"
    elif variant == "Variant 2 — Roulette Wheel + 1/2-Point + Children":
        return "Roulette Wheel", "1-Point + 2-Point", "Children"
    elif variant == "Variant 3 — Roulette Wheel + 1/2-Point + Elitist":
        return "Roulette Wheel", "1-Point + 2-Point", "Elitist"
    else:
        return sel, cross, rep
...
if btn_ga_eval:
    if float(ga_alpha) == 0.0:
        ...
        eff_sel, eff_cross, eff_rep = _resolve_ga_ops(
            str(ga_variant),
            str(ga_selection),
            str(ga_crossover),
            str(ga_replacement),
        ...
        for r in range(int(ga_runs)):
            sol, fit_val, acc_val, sel_idx, h_best, h_avg, traj_x1, final_pop = GA_FS(
                ...
                Rc=float(ga_Rc),
                Rm=float(ga_Rm),
                selection_method=eff_sel,
                crossover_method=eff_cross,
                replacement_method=eff_rep,
                sf_mode=str(ga_sf_mode),
                SF=int(ga_SF) if ga_sf_mode == "Fixed" else None,
            )
        ...
        st.session_state.ga_variant_used  = ga_variant
...
if st.session_state.ga_solution is not None:
    ...
    ga_var_used  = st.session_state.get("ga_variant_used", "—")

    ga_sol_str = " | ".join([str(int(v)) for v in ga_sol])
    ga_idx_str = " | ".join([str(i) for i in ga_sel_idx])

    st.caption(f"Results for: **{ga_var_used}**")

    ...
    with left_col:
        ...
        with fs_alpha_col:
            ...
            alpha_fs = st.number_input(
                "α", min_value=0.0, max_value=1.0, value=0.99, step=0.01,
...
if "fs_solution" not in st.session_state:
    ...
    st.session_state.fs_SF             = np.random.random_integers(1,30)
...
if st.session_state.fs_solution is not None:
    ...
    with res_col2:
        with st.container(border=True):
            ...
            st.markdown(f"**Mean** — {np.mean(gbest_arr):.3f},")
